...
// Define schemas for the structured outputs
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert Research Copilot. Your goal is to generate a comprehensive content package based on the provided topic. This package must include a detailed document, a set of presentation slides, and a timeline of key events. The output MUST be a single, valid JSON object that adheres to the specified output schema.
  ...
  **Instructions:**
  ...
  1.  **Generate Document Content ('documentContent')**:
      ...
      *   If the topic involves technical or scientific concepts, include complex mathematical formulas using standard LaTeX syntax. Wrap inline formulas in \\\\( ... \\\\) and block formulas in \\\\[ ... \\\\].
